...
import com.ghanshyam.shopshop.Helper.TinyDB
...
class ManagementCart(val context: Context) {
    ...
    fun insertFood(item: ItemsModel) {
        ...
        if (existAlready) {
            listFood[index].itemsInCart = item.itemsInCart
        } else {
            listFood.add(item)
        }
    ...
    fun minusItem(listFood: ArrayList<ItemsModel>, position: Int, listener: ChangeNumberItemsListener) {
        if (listFood[position].itemsInCart == 1) {
            listFood.removeAt(position)
        } else {
            listFood[position].itemsInCart--
        }
        tinyDB.putListObject("CartList", listFood)
        listener.onChanged()
    }
    ...
    fun getTotalFee(): Double {
        ...
        for (item in listFood) {
            fee += item.price * item.itemsInCart
        }
        return fee
    }
}